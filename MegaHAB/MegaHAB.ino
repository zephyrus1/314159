...
//APRS send, NOTE: needs sufficient satellite count
  ...
  Serial.println(String(data)); //data = hour:minute:second,millis/1000,lat(deg),lon(deg),GPS alt(m),number of sats,external sensor temp(C),pressure(hPa),barometer alt (m),x_accel,y_accel,z_accel,x_mag,y_mag,z_mag,z_rate,y_rate,x_rate,roll,pitch,yaw,cut1,cut2,cut3
