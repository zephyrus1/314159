...
const int CUT_1_TIMER = -999;//FIX this //msecs DROPPER
const int CUT_2_TIMER = -999;//fix this //msecs REFLECTOR
  ...
  */
  ...
  if (cut_1_progress == 1 && (millis() - start_1_time) >= CUT_TIME) {
    ...
    if (SD.exists("log.csv")) {
      Serial.println("Writing data to log file...");
      if (logFile = SD.open("log.csv", FILE_WRITE)) {
        logFile.println("Ending cut for droppers");
        logFile.close();
      }
      else {
        Serial.println("ERROR: UNABLE TO OPEN LOG FILE");
  ...
  if ((alt >= CUT_2_ALT || millis() >= CUT_2_TIMER) && cut_2_progress == 0) {
    ...
    if (SD.exists("log.csv")) {
      Serial.println("Writing data to log file...");
      if (logFile = SD.open("log.csv", FILE_WRITE)) {
        logFile.println("Starting cut for reflector");
        logFile.close();
      }
      else {
        Serial.println("ERROR: UNABLE TO OPEN LOG FILE");
  ...
  if (cut_2_progress == 1 && (millis() - start_2_time) >= CUT_TIME) {
    ...
    if (SD.exists("log.csv")) {
      Serial.println("Writing data to log file...");
      if (logFile = SD.open("log.csv", FILE_WRITE)) {
        logFile.println("Ending cut for reflector");
        logFile.close();
      }
      else {
        Serial.println("ERROR: UNABLE TO OPEN LOG FILE");
